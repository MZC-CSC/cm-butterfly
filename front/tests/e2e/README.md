...
> | 작성 | 2026-07-05 (테스트자동화 [BAR-875](https://mzdevs.atlassian.net/browse/BAR-875)·[BAR-879](https://mzdevs.atlassian.net/browse/BAR-879)·[BAR-880](https://mzdevs.atlassian.net/browse/BAR-880)) |
...
2. **셀렉터는 data-testid 우선** — 프론트 요소에 `data-testid`를 부여([BAR-880](https://mzdevs.atlassian.net/browse/BAR-880))하고 Page Object는 `getByTestId(...)`를 쓴다. 미부여 구간은 `.or(placeholder/role)` fallback으로 두되, 부여 후 fallback 제거. **텍스트·클래스·nth 셀렉터 지양**(다국어·소스 변경에 취약).
...
BASE_URL=http://cmig.stage.cscmzc.com npx playwright test   # 대상 서버 지정
```
...
| stage(운영) | `http://cmig.stage.cscmzc.com` | 배포된 현행화 butterfly 회귀 |
...
- **계획서 §7 TC 목록**(national-project-note `웹테스트 자동화/웹 테스트 자동화 계획.md`) → 기능 `.feature`로 이관(TC-AUTH·프로젝트·워크플로우·클라우드리소스·모델).
