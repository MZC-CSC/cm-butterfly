...
seg_label() {
  case "${1#seg}" in
    ...
    2x) SEG="seg2x-소스서비스등록-개별2건" ;;
    3) SEG="seg3-트랙1-추천그대로-인프라생성" ;;
    4) SEG="seg4-트랙2-타겟모델에5555와스펙-인프라생성" ;;
    5) SEG="seg5-트랙3-워크플로우복제해포트와스펙변경-인프라생성" ;;
    6) SEG="seg6-트랙4-소스모델에5555-추천따라오는지-인프라생성" ;;
    6b) SEG="seg6b-워크로드-인프라4대확인" ;;
    7) SEG="seg7-소프트웨어수집-SW모델-마이그레이션" ;;
    8) SEG="seg8-nginx설치확인-부하테스트" ;;
    9) SEG="seg9-정리-인프라삭제" ;;
    13) SEG="seg13-서비스상태-정상" ;;
    14) SEG="seg14-서비스장애-알림과복구" ;;
...
for v in "${VIDEOS[@]}"; do
  ...
  # 자르기 전 원본을 남긴다.
  #
  # * 정지 제거는 제자리에서 덮어쓰므로, 한 번 자르면 무엇이 잘렸는지 확인할 길이 없다. 실제로
  #   알림 애니메이션이 잘려 나갔는데 원본이 없어 "떴는지 안 떴는지"부터 다시 따져야 했다
  #   (2026-08-19). 원본이 있으면 자른 결과가 이상할 때 기준을 다시 잡고 다시 자를 수 있다.
  if [ "${E2E_KEEP_ORIGINAL:-1}" = "1" ]; then
    mkdir -p "$KEEP_DIR/원본"
    cp "$KEEP_DIR/$name.mp4" "$KEEP_DIR/원본/$name.mp4"
    echo "[keep] 원본 보관: 원본/$name.mp4"
  fi
