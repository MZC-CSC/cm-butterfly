...
if [ "${#SEGMENTS[@]}" -eq 0 ]; then
  ...
  #
  # 구간2a(파일로 한 번에 등록) 도 빠져 있다 — 파일 고르는 창이 이 장면의 요점인데 그 창은
  # 운영체제가 그린다. 여기서 찍으면 GTK 창이 나오고, 그 창이 여는 폴더가 이 장비의 홈이라
  # 개발 폴더 목록이 화면에 남는다(폴더를 바꾸는 방법 세 가지가 모두 막혔다). 그래서 이 한
  # 구간만 윈도우즈에서 따로 찍어 완성본에 끼운다 — 절차는 워크플로우 저장소의
  # conf/private/윈도우즈-촬영/실행방법.md. 시나리오 자체는 살아 있으므로 기능 확인에는
  # 그대로 쓴다:  scripts/record-x11.sh seg2a
  SEGMENTS=(1 0 2 3 4 5 6 6b 7 8 8b 9 10 13)
fi
...
#
# ★ 이어 찍을 때 이것을 빠뜨리면 **앞 구간이 만들어 둔 것이 지워진다.** 인프라는 남으므로
#   "자원은 그대로인데 소스 서비스만 사라진" 상태가 되고, 그것을 쓰는 구간부터 연쇄로 죽는다.
#   2026-08-24 그렇게 구간7 을 날렸다 — 이 줄이 여기 있는데도 보지 않고 불렀다.
#
#   같이 넘길 것: E2E_RUN_ID 도 그 벌의 값으로 준다. 없으면 이름이 새로 정해져 앞 구간이 만든
#   것을 찾지 못한다.
