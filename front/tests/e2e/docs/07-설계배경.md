...
│   ├─ scenarios/마이그레이션전체흐름.feature  @scenario @e2e @migration
│   └─ scenarios/워크플로우툴-파라미터변경.feature @scenario @e2e
...
2. **셀렉터는 data-testid 우선** — 프론트 요소에 `data-testid`를 부여하고 Page Object는 `getByTestId(...)`를 쓴다. 미부여 구간은 `.or(placeholder/role)` fallback으로 두되, 부여 후 fallback 제거. **텍스트·클래스·nth 셀렉터 지양**(다국어·소스 변경에 취약).
...
## 10. 플래그십 시나리오 — 마이그레이션 전체 흐름 (인프라 + 소프트웨어)

**워크플로우 실행·EC2 프로비저닝**에 더해, 소스에서 돌던 **소프트웨어가 타깃으로 옮겨지는지**까지 한 흐름으로 확인한다.
...
### 왜 하나의 시나리오인가 — 두 마이그레이션은 순서가 있다

`grasshopper_task_software_migration` 은 **설치 대상 인프라를 query 파라미터(`nsId`·`infraId`)로 받는다.**
타깃 SW 모델은 "무엇을 설치할지"만 알고 "어디에 설치할지"는 모르기 때문이다.

그래서 소프트웨어 마이그레이션은 **인프라 마이그레이션이 먼저 끝나 있어야** 하고, 그렇게 만들어진
인프라의 id를 **워크플로우 툴에서 채워 넣어야** 비로소 실행된다. 두 단계를 갈라 놓으면 뒤 시나리오가
앞 시나리오의 산출물에 몰래 기대게 된다 — 그래서 한 흐름으로 둔다.

> ★ nginx는 **마이그레이션이 올린다.** 예전에는 cb-tumblebug 원격명령으로 타깃에 강제 설치했는데, 그건
> 소프트웨어 마이그레이션이 검증되지 않던 시절의 우회였다. 지금은 제품의 정식 경로로 올리고, 실제로
> 올라갔는지는 타깃 노드에 직접 물어 확인한다. 그 nginx가 곧 부하테스트 대상이 된다.
>
> 그러려면 **소스 서버에 옮길 소프트웨어가 실제로 있어야 한다** — `scripts/prepare-source-server.sh`.

> 비용 보호: 타깃 스펙은 nano/small급으로 강제하고, 생성 자원은 시나리오 종료 시 정리한다. dev/stage 서버 `terminate`는 금지.

---

### 정리(teardown)는 Normal Delete 로 — Force 는 자원을 버리고 간다

워크로드 삭제 모달에는 두 가지가 있다.

| 삭제 방식 | cb-tumblebug 옵션 | 실제 동작 |
|-----------|------------------|-----------|
| **Normal Delete** | `option=terminate` | **VM을 종료하고** 인프라를 지운다 |
| Force Delete | `option=force` | **CSP 삭제 결과와 무관하게 메타데이터만** 지운다 (모달 경고 배너에 그대로 적혀 있다) |

한동안 정리 단계가 Force 를 쓰고 있었다. 목록에서는 사라지는데 **EC2는 계속 떠서 요금을 먹는다.**
그렇게 버려진 인스턴스가 여러 대 돌고 있었다.

그러니 정리는 **Normal Delete** 로 한다. "정리했다"는 판정은 *자원이 정말 없어졌다*는 뜻이어야 한다.
삭제 확인(`expectInfraGone`)도 **전 페이지를 훑은 뒤에만** "없다"고 판정한다 — 목록은 15행씩 끊기므로
1페이지만 보면 뒤 페이지에 살아 있는 인프라를 지웠다고 오판한다.
