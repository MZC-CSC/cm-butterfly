...
export const descriptions = {
  sourceModel5555:
    '소스 모델에 방화벽 포트 추가\n소스 모델에 오픈하고 싶은 방화벽 포트를 미리 추가해 놓으면 타겟 모델과 워크플로우에서 이 설정을 그대로 사용함.',
  targetModelRecommended: '추천받은 구성을 수정없이 그대로 사용함',
  targetModel5555:
    '타겟 모델에 방화벽 포트 추가\n타겟 모델에 오픈하고 싶은 방화벽 포트를 미리 추가해 놓으면 워크플로우에서 이 설정을 그대로 사용함',
  targetModelSpecUp:
    '타겟 모델에 방화벽 포트 추가 및 인스턴스 스펙 변경\n타겟 모델에 오픈하고 싶은 방화벽 포트를 추가하고 사용하고 싶은 인스턴스 스펙으로 변경하면 워크플로우에서 이 설정을 그대로 사용함',
  softwareTargetModel:
  ...
  infraWorkflow5555:
    '5555 방화벽 포트가 추가된 타겟 모델로 생성된 인프라 마이그레이션 워크플로우\n모델에 미리 추가한 5555 방화벽 포트도 개방된다. 워크플로우에서 자원 이름 접두어를 직접 지정했으며, 만들어지는 자원 이름에 그대로 붙는다.',
  infraWorkflow5555SpecUp:
    '5555 방화벽 포트 추가와 인스턴스 스펙이 변경된 타겟 모델로 생성된 인프라 마이그레이션 워크플로우\n모델에 미리 추가한 5555 방화벽 포트가 개방되고, 변경한 스펙으로 인스턴스가 생성된다. 워크플로우에서 자원 이름 접두어를 직접 지정했으며, 만들어지는 자원 이름에 그대로 붙는다.',
  softwareWorkflow:
    '소프트웨어 마이그레이션 워크플로우\n타겟 SW 모델이 무엇을 설치할지 정하고, 어느 인프라에 설치할지는 이 워크플로우에서 지정함',
} as const;
