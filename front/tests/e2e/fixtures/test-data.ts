...
export const descriptions = {
  ...
  infraWorkflow5555:
    '5555 방화벽 포트가 추가된 타겟 모델로 생성된 인프라 마이그레이션 워크플로우. 모델에 미리 추가한 5555 방화벽 포트도 개방된다.',
  ...
  infraWorkflowSpecEdited:
    '워크플로우에서 인스턴스 스펙을 직접 변경한 인프라 마이그레이션 워크플로우. 타겟 모델의 스펙 대신 워크플로우에서 지정한 스펙으로 인스턴스가 생성되며, 모델에 미리 추가한 5555 방화벽 포트도 개방된다.',
  softwareWorkflow:
