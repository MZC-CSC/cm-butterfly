...
export const descriptions = {
  sourceModel5555:
    '소스 모델 방화벽 포트 추가 테스트. 수집된 정보에 열어 두고 싶은 포트를 미리 넣어 두면 타깃 모델과 워크플로우가 이 설정을 그대로 받아 간다',
  targetModelRecommended:
    '추천받은 구성을 그대로 저장한 기본 타깃 모델. 포트나 스펙을 손보기 전 상태로, 뒤에서 만드는 모델들과 비교하는 기준이 된다',
  targetModel5555:
    '타깃 모델 방화벽 포트 추가 테스트. 추천받은 구성에 추가로 열고 싶은 포트를 넣어 둔 샘플 모델로, 워크플로우가 이 설정 그대로 인프라를 만든다',
  targetModelSpecUp:
    '타깃 모델 방화벽 포트 추가 및 인스턴스 스펙 변경 테스트. 열고 싶은 포트를 추가하고 스펙을 한 단계 올려 둔 샘플 모델로, 워크플로우가 이 구성 그대로 인프라를 만든다',
  softwareTargetModel:
    '소프트웨어 마이그레이션용 타깃 모델. 옮길 소프트웨어만 담고, 어느 인프라에 설치할지는 워크플로우에서 지정한다',
} as const;
