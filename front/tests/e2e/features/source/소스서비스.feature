...
기능: 소스 서비스 (소스 컴퓨팅)
  ...
  @unit
  시나리오: 연결정보 익스포트 (선택한 연결만 임포트 양식으로 내려받기)
    먼저 소스 서비스 화면을 연다
    만약 "e2e-export-group" 소스그룹에 "e2e-export-conn" 연결정보를 등록하면
    그리고 "e2e-export-group" 소스그룹을 선택한다
    그러면 익스포트 버튼이 비활성이다
    만약 "e2e-export-conn" 연결정보를 체크한다
    그러면 익스포트 버튼이 활성이다
    만약 익스포트를 누르면
    그러면 암호화 컬럼 안내가 보인다
    만약 익스포트를 확인하면
    그러면 "e2e-export-conn" 이름으로 시작하는 파일이 내려받아진다
