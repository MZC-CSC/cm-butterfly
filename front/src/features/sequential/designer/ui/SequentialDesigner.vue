...
.sqd-step-launch-pad:has(> g[class^='sqd-step-'] ~ g[class^='sqd-step-'])
  ...
  /*
    ...
    유일한 클릭 지점이라 상자를 고르지도 지우지도 못하게 된다. 색만 지운다.

    집는 자리는 오히려 **넓힌다.** 원래 굵기가 2px 이라 보일 때조차 겨냥하기
    어려웠는데, 보이지 않게 되면 사실상 맞출 수 없다(실제로 아무리 눌러도
    안 잡힌다는 이야기가 나왔다). 12px 띠로 넓혀 테두리 근처를 누르면 잡히게
    한다 — 투명이라 보이지는 않고, `pointer-events: stroke` 라 색과 무관하게
    집힌다.
  */
  stroke: transparent;
  stroke-width: 12px;
